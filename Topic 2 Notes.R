...
x <- seq(0, max(total), length.out=n) # Creates a sequence of n evenly spaced values from 0 to the maximum value in total.
y <- dnorm(x, mean(total), sd(total)) #Computes the normal density values for the sequence x using the mean and standard deviation of total.
